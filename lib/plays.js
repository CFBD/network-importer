                    ...
                    for (let i = 0; i < playType.types.length; i++) {
                        ...
                        if (match) {
                            ...
                            if (!athlete && (match.indexOf('Jr.') !== -1 || match.indexOf('II') !== -1 || match.indexOf('IV' !== -1) || match.indexOf('III' != -1))) {
                                athlete = players.find(p => p.athlete == match.replace(' Jr.', '').replace('II', '').replace(' IV', '').replace(' III', '') && (!statType.isOffense || play.offense_id == p.team_id) && (statType.isOffense || play.defense_id == p.team_id));
                            }
